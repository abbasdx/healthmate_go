...
const PayementStep = ({
      ...
      const options = {
        ...
        modal: {
          ondismiss: () => {
            ...
            // Auto-reopen payment modal only on first close
            if (modalCloseCountRef.current === 1) {
              setTimeout(() => {
                handlePayment();
              }, 1000); // 1 second delay before reopening
            } else {
              // Disable auto-open after first close
  ...
  const handlePaynow = () => {
    if (appointmentId && patientName) {
      // Reset modal close count and enable auto-open when manually clicking the button
      ...
      {paymentStatus === "idle" && (
        <div className="flex justify-between gap-2">
          ...
          >
            ...
            ) : appointmentId && patientName && shouldAutoOpen ? (
              <>
                <Loader2 className="w-5 h-5 mr-2 animate-spin" />
                <span className="text-sm md:text-lg">
                  Opening Payment...
