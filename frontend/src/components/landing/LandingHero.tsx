...
const LandingHero = () => {
  ...
  const handleCategoryClick = (categorytitle: string) => {
    if (isAuthenticated) {
      router.push(`/doctors-list?category=${categorytitle}`);
    } else {
      router.push(`/signup/patient?category=${categorytitle}`);
    }
  };

  const handleAiSymptomsChecker = () => {
    router.push('/ai');
  };
      ...
      <div className="container mx-auto text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center mb-12">
          ...
          <Button
            size="lg"
            variant="outline"
            className="border-purple-600 text-purple-600 hover:bg-purple-50 rounded-full px-8 py-3 text-lg"
            onClick={handleAiSymptomsChecker}
          >
            AI Symptoms Checker
          </Button>
